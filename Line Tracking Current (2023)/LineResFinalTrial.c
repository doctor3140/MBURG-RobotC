...
task display() //Display for Error Log
{
	...
	displayBigTextLine(6, "Reflect: %d", SensorValue[S3]);
	displayBigTextLine(8, "Sonar: %d", SensorValue[S3]);
...
task main()
{
	...
	repeat(forever)
	{
	  ...
	  if((SensorValue[S3]>=60)){ //if it is silver
			encoderArmDown(135);
			mowRoom1();
		}
		else if(SensorValue[S4] <= 5) avoidObstacle();//basically the entire program
		lineTracking();
		//sweepRoom();
	}
}
